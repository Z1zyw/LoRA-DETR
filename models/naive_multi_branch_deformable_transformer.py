...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage=False, two_stage_num_proposals=300, look_forward_twice=False, mixed_selection=False, use_ms_detr=False, use_aux_ffn=True, num_aux=1):
        super().__init__()
        ...
        decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, dec_n_points, use_ms_detr=use_ms_detr, use_aux_ffn=use_aux_ffn, num_aux=num_aux)
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, use_ms_detr=False, use_aux_ffn=True, num_aux=1):
        super().__init__()
        ...
        if NAIVE_IMPL and num_aux > 1:
            # assert num_aux <= 3
            # auxiliary ffn: second ffn
            ...
            # auxiliary ffn: third ffn
            if num_aux == 3:
                self.linear7 = nn.Linear(d_model, d_ffn)
                self.dropout9 = nn.Dropout(dropout)
                self.linear8 = nn.Linear(d_ffn, d_model)
            ...
            if num_aux >= 5:
                ...
                self.linear9 = nn.Linear(d_model, d_ffn)
                self.dropout11 = nn.Dropout(dropout)
                self.linear10 = nn.Linear(d_ffn, d_model)
                self.dropout12 = nn.Dropout(dropout)
                self.norm7 = nn.LayerNorm(d_model)
                
                self.linear11 = nn.Linear(d_model, d_ffn)
                self.dropout13 = nn.Dropout(dropout)
                self.linear12 = nn.Linear(d_ffn, d_model)
                self.dropout14 = nn.Dropout(dropout)
                self.norm8 = nn.LayerNorm(d_model)
                if num_aux == 6:
                    self.linear13 = nn.Linear(d_model, d_ffn)
                    self.dropout15 = nn.Dropout(dropout)
                    self.linear14 = nn.Linear(d_ffn, d_model)
                    self.dropout16 = nn.Dropout(dropout)
                    self.norm9 = nn.LayerNorm(d_model)
                else:
                    raise NotImplementedError("Not implemented auxiliary ffn for num_aux > 3 and not >=5")
    ...
    def forward_ffn_naive(self, tgt):
        tgt2 = self.linear6(self.dropout7(self.activation(self.linear5(tgt))))
        tgt = tgt + self.dropout8(tgt2)
        tgt = self.norm5(tgt)
        return tgt
    
    def forward_ffn_naive2(self, tgt):
        tgt2 = self.linear8(self.dropout9(self.activation(self.linear7(tgt))))
        tgt = tgt + self.dropout10(tgt2)
        tgt = self.norm6(tgt)
        return tgt

    def forward_ffn_naive3(self, tgt):
        tgt2 = self.linear10(self.dropout11(self.activation(self.linear9(tgt))))
        tgt = tgt + self.dropout12(tgt2)
        tgt = self.norm7(tgt)
        return tgt
    
    def forward_ffn_naive4(self, tgt):
        tgt2 = self.linear12(self.dropout13(self.activation(self.linear11(tgt))))
        tgt = tgt + self.dropout14(tgt2)
        tgt = self.norm8(tgt)
        return tgt

    def forward_ffn_naive5(self, tgt):
        tgt2 = self.linear14(self.dropout15(self.activation(self.linear13(tgt))))
        tgt = tgt + self.dropout16(tgt2)
        tgt = self.norm9(tgt)
        return tgt

    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        if self.use_ms_detr and not NAIVE_IMPL: 
            # cross attention
        ...
        elif self.use_ms_detr and NAIVE_IMPL:
            q = k = self.with_pos_embed(tgt, query_pos)
            tgt2 = self.self_attn(q.transpose(0, 1), k.transpose(0, 1), tgt.transpose(0, 1))[0].transpose(0, 1)
            tgt = tgt + self.dropout2(tgt2)
            tgt = self.norm2(tgt)
            ...
            if self.num_aux == 2:
                tgt_o2m2 = self.forward_ffn_naive(tgt)
                tgt_o2m = [tgt_o2m, tgt_o2m2]
                tgt_o2m = torch.stack(tgt_o2m, dim=0)
            
            elif self.num_aux == 3:
                tgt_o2m2 = self.forward_ffn_naive(tgt)
                tgt_o2m3 = self.forward_ffn_naive2(tgt)
                tgt_o2m = [tgt_o2m, tgt_o2m2, tgt_o2m3]
                tgt_o2m = torch.stack(tgt_o2m, dim=0)
                
            elif self.num_aux == 5:
                tgt_o2m2 = self.forward_ffn_naive(tgt)
                tgt_o2m3 = self.forward_ffn_naive2(tgt)
                tgt_o2m4 = self.forward_ffn_naive3(tgt)
                tgt_o2m5 = self.forward_ffn_naive4(tgt)
                tgt_o2m = [tgt_o2m, tgt_o2m2, tgt_o2m3, tgt_o2m4, tgt_o2m5]
                tgt_o2m = torch.stack(tgt_o2m, dim=0)
                ...
                tgt_o2m4 = self.forward_ffn_naive3(tgt)
                tgt_o2m5 = self.forward_ffn_naive4(tgt)
                tgt_o2m6 = self.forward_ffn_naive5(tgt)
                tgt_o2m = [tgt_o2m, tgt_o2m2, tgt_o2m3, tgt_o2m4, tgt_o2m5, tgt_o2m6]
                tgt_o2m = torch.stack(tgt_o2m, dim=0)
            
            
            else:
                tgt_o2m = tgt_o2m.unsqueeze(0)

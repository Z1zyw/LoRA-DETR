...
def rank_wise_decay(repeat_num, rank_decay):
    rank_weights = torch.cat([rank_decay[i] ** (repeat_num[i] - 1 - torch.arange(repeat_num[i], device=repeat_num.device)) for i in range(len(repeat_num))], dim=0)
    return rank_weights

def get_rank_wise_cost(pr_inds, gt_inds, cost_matrix):
    try:
        RANK_ALPHA = 0.5
        # from small to large
        gt_id, gt_cnt = gt_inds.unique(return_counts=True)
        rank_decay = RANK_ALPHA ** (1. / gt_cnt)
        rank_weights = rank_wise_decay(gt_cnt, rank_decay)
        cost_matrix[gt_inds, pr_inds] = cost_matrix[gt_inds, pr_inds] * rank_weights
        return cost_matrix
    except:
        return cost_matrix
...
class Matcher(object):
    ...
    def __call__(self, match_quality_matrix):
        ...
        if self.allow_low_quality_matches:
            ...
            # sharp_quality = self.set_low_quality_matches_sparse(match_labels, match_quality_matrix, matches, matched_vals)
    ...
    def set_low_quality_matches_sparse(self, match_labels, match_quality_matrix, matches, matched_vals,k=1):
        # set_low_quality_matches_ is for dense queries 
        # we replace it with set_low_quality_matches_sparse

        sharp_quality = torch.zeros_like(match_quality_matrix)
        sharp_quality[matches, torch.arange(match_quality_matrix.shape[1])] = matched_vals
        self.set_low_quality_matches_(match_labels, sharp_quality, k=1)
        return sharp_quality
...
class Stage2Assigner(nn.Module):
    ...
    def update_cost_class(self, coef_cls):
        """
        Update the coefficient of classification cost.
        """
        self.coef_cls = coef_cls
        self.coef_box = 1 - coef_cls
    ...
    # implement for single image (without batch)
    @ torch.no_grad()
    def forward(self, pred_boxes, pred_logits, gt_boxes, gt_classes, return_cost_matrix=False, rank_wise_cost=False):
        ...
        # sharp_cost = cost_matrix.new_full(cost_matrix.shape, 1e-9)
        # sharp_cost = cost_matrix.clone() * 0.5
        # sharp_cost[pos_gt_inds, pos_pr_inds] = cost_matrix[pos_gt_inds, pos_pr_inds]
        sharp_cost = None
        if sharp_cost is not None:
            pos_pr_inds, pos_gt_inds = self.postprocess_indices(pos_pr_inds, pos_gt_inds, sharp_cost, self.k)
        else:
            pos_pr_inds, pos_gt_inds = self.postprocess_indices(pos_pr_inds, pos_gt_inds, cost_matrix, self.k)

        indices = (pos_pr_inds, pos_gt_inds)
        if rank_wise_cost:

...
# 这部分的逻辑没有使用原始 的 pr_inds, 而是直接根据 gt_inds 来进行 topk 采样
...
class Matcher(object):
    ...
    def __call__(self, match_quality_matrix):
        ...
        # return matches, match_labels, sharp_quality
...
class Stage2Assigner(nn.Module):
    def __init__(self, threshold=0.4, k=6, coef_box=0.7, coef_cls=0.3, positive_fraction=0.25):
        super().__init__()
    ...
    def forward(self, outputs, targets, return_cost_matrix=False, rank_wise_cost=False):
...
# define functions about basic matcher modified from Matcher
def set_low_quality_matches__(match_labels, match_quality_matrix, k=1):
    highest_quality_foreach_gt_inds = match_quality_matrix.topk(k=k, dim=1)[1]
    match_labels[highest_quality_foreach_gt_inds.flatten()] = 1

# get matches from single threshold, 0 is negative, 1 is positive
def get_matches_from_quality_matrix(match_quality_matrix, threshold, allow_low_quality_matches=True):
    assert match_quality_matrix.dim() == 2
    if match_quality_matrix.numel() == 0:
        default_matches = match_quality_matrix.new_full((match_quality_matrix.size(1),), 0, dtype=torch.int64)
        default_match_labels = match_quality_matrix.new_full((match_quality_matrix.size(1),), 0, dtype=torch.int8)
        return default_matches, default_match_labels
    ...
    index = matched_vals >= threshold
    match_labels[index] = 1
    
    if allow_low_quality_matches:
        # set_low_quality_matches__(match_labels, match_quality_matrix)
        highest_quality_foreach_gt_inds = match_quality_matrix.topk(k=1, dim=1)[1]
        match_labels[highest_quality_foreach_gt_inds.flatten()] = 1
    ...
    def get_cost_matrix(
        ...
        assert coef_box is not None and coef_cls is not None
        num_queries = len(pred_logits)
        out_prob = pred_logits.sigmoid()
        out_bbox = pred_boxes
        cost_box = box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(gt_boxes))[0]
        cost_class = out_prob[:, gt_classes]
        C = coef_box * cost_box + coef_cls * cost_class
        C = C.view(num_queries, -1)
    ...
    def forward(
        ...
        if match_k is None:
            match_k = torch.ones(bs, dtype=torch.int64, device=outputs['pred_logits'].device) * 6
        if match_threshold is None:
            match_threshold = torch.ones(bs, dtype=torch.float32, device=outputs['pred_logits'].device) * 0.4
        if match_cls_coef is None:
            match_cls_coef = torch.ones(bs, dtype=torch.float32, device=outputs['pred_logits'].device) * 0.3
        if match_box_coef is None:
            match_box_coef = torch.ones(bs, dtype=torch.float32, device=outputs['pred_logits'].device) * 0.7
        
        assert len(match_k) == bs
        assert len(match_threshold) == bs
        ...
        with torch.no_grad():
            for b in range(bs):
                ...
                cost_matrix = self.get_cost_matrix(
                    ...
                    gt_classes, 
                    gt_boxes,
                    coef_box=match_box_coef[b], 
                    coef_cls=match_cls_coef[b]
                )
                
                # get matches
                matched_idxs, matched_labels = get_matches_from_quality_matrix(
                    cost_matrix, 
                    match_threshold[b], 
                ...
                sampled_idxs, sampled_gt_classes = self._sample_proposals(
                    ...
                    matched_labels, 
                    targets[b]['labels'], 
                    batch_size_per_image=num_queries
                )
                ...
                if use_sharp_cost:
                    sharp_cost = cost_matrix.new_full(cost_matrix.shape, 0.)
                    sharp_cost[pos_gt_inds, pos_pr_inds] = cost_matrix[pos_gt_inds, pos_pr_inds]
                    pos_pr_inds, pos_gt_inds = self.postprocess_indices(pos_pr_inds, pos_gt_inds, sharp_cost, match_k[b])                
                else:
                    pos_pr_inds, pos_gt_inds = self.postprocess_indices(pos_pr_inds, pos_gt_inds, cost_matrix, match_k[b])
                
                indices.append((pos_pr_inds, pos_gt_inds))
                
                if rank_wise_cost:
                    cost_matrix = get_rank_wise_cost(pos_pr_inds, pos_gt_inds, cost_matrix)
                cost_matrices.append(cost_matrix)
    ...
    def __call__(self, match_quality_matrix, k_p2g):
        assert match_quality_matrix.dim() == 2
        if match_quality_matrix.numel() == 0:
            default_matches = match_quality_matrix.new_full(
                (match_quality_matrix.size(1),), 0, dtype=torch.int64
        ...
        # Max over gt elements (dim 0) to find best gt candidate for each prediction
        # matched_vals, matches = match_quality_matrix.max(dim=0)
        matched_vals, matches = match_quality_matrix.topk(k=k_p2g, dim=0)
        match_labels = matches.new_full(matches.size(), 1, dtype=torch.int8)
    ...
    def set_low_quality_matches_(self, match_labels, match_quality_matrix, k=1):
        ...
        """
        highest_quality_foreach_gt_inds = match_quality_matrix.topk(k=k, dim=1)[1]
        match_labels[0, highest_quality_foreach_gt_inds.flatten()] = 1
    
       
    # 目前实现的 Labels_o2m_v2 loss 实际上是一种 many 2 many 的 label
    def __init__(self, k_p2g=2, 
                 k_g2p=6, 
                 threshold=0.4, 
                 coef_box=0.7, 
                 coef_cls=0.3, 
                 positive_fraction=1.0):
        super().__init__(threshold, k_g2p, coef_box, coef_cls, positive_fraction)
        self.k_p2g = k_p2g
        self.proposal_matcher = Many2ManyBaseMatcher(
    ...
    def forward(self, outputs, targets, return_cost_matrix=False, rank_wise_cost=False):
        ...
        with torch.no_grad():
            for b in range(bs):
                ...
                k_p2g = min(self.k_p2g, len(gt_boxes))
                matched_idxs, matched_labels = self.proposal_matcher(cost_matrix, k_p2g)

                pos_pr_inds_all, pos_gt_inds_all = [], []
                for i in range(k_p2g):
                    sampled_idxs, sampled_gt_classes = self._sample_proposals(
                        matched_idxs[i], matched_labels[i], targets[b]['labels'], batch_size_per_image=num_queries
                    )

                    pos_pr_inds = sampled_idxs[sampled_gt_classes != self.bg_label]
                    pos_gt_inds = matched_idxs[i][pos_pr_inds]
                    # TODO 这部分的逻辑比较怪，考虑重构。尤其是 postprocess_indices 的逻辑
                    
                    masked_cost_matrix = cost_matrix.new_full(cost_matrix.shape, -1e9)
                    masked_cost_matrix[pos_gt_inds, pos_pr_inds] = cost_matrix[pos_gt_inds, pos_pr_inds]
                    
                    pos_pr_inds_, pos_gt_inds_ = self.postprocess_indices(pos_pr_inds, pos_gt_inds, masked_cost_matrix, self.k)
                    # masked_cost_matrix[pos_gt_inds_, pos_pr_inds_] = -1e9  # mask out the used indices

                    pos_pr_inds_all.append(pos_pr_inds_)
                    pos_gt_inds_all.append(pos_gt_inds_)

                if len(pos_pr_inds_all) == 0:
                    indices.append((torch.tensor([], dtype=torch.int64, device=cost_matrix.device), 
                                    torch.tensor([], dtype=torch.int64, device=cost_matrix.device)))
                else:
                    indices.append((torch.cat(pos_pr_inds_all, dim=0), torch.cat(pos_gt_inds_all, dim=0)))

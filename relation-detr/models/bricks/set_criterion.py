...
class HybirdSetCriterionWithO2M(HybridSetCriterion):
    def __init__(
        self,
        num_classes: int,
        matcher: nn.Module,
        matcher_enc: nn.Module, 
        matcher_o2m: nn.Module,
        weight_dict: Dict,
        alpha: float = 0.25,
        gamma: float = 2.0,
        two_stage_binary_cls: bool = False,
        use_indices_merge: bool = True,
    ):
        super().__init__(
        ...
        self.matcher_enc = matcher_enc
        self.use_indices_merge = use_indices_merge
    
    # override the calculate_loss to save the indices for o2m loss
    def calculate_loss(self, outputs, targets, num_boxes, indices=None, **kwargs):
        losses = {}
        # get matching results for each image
        if not indices:
        ...
        losses.update(loss_class)
        losses.update(loss_boxes)
        losses.update({"o2o_indices": indices})
        return losses
    
    @staticmethod
    # modified from MS-DETR
    ...
    def calculate_enc_anchors_loss(self, outputs, targets, num_boxes, indices=None, **kwargs):
        losses = {}
        # get matching results for each image
        if not indices:
            gt_boxes, gt_labels = list(zip(*map(lambda x: (x["boxes"], x["labels"]), targets)))
            # get anchors not pred_boxes
            pred_logits, pred_boxes = outputs["pred_logits"], outputs["anchors"]
            indices = list(map(self.matcher_enc, pred_boxes, pred_logits, gt_boxes, gt_labels))
    ...
    def calculate_o2m_loss(self, outputs, targets, num_boxes, indices=None, o2o_indices=None, matcher_idx=0, **kwargs):
        losses = {}
        # get matching results for each image
        if not indices:
            gt_boxes, gt_labels = list(zip(*map(lambda x: (x["boxes"], x["labels"]), targets)))
            pred_logits, pred_boxes = outputs["pred_logits"], outputs["pred_boxes"]
            indices = list(map(self.matcher_o2m[matcher_idx], pred_boxes, pred_logits, gt_boxes, gt_labels))
            if o2o_indices is not None:
                indices = self.indices_merge(pred_boxes.shape[1], o2o_indices, indices)
        loss_class = self.loss_labels(outputs, targets, num_boxes, indices=indices)
        loss_boxes = self.loss_boxes(outputs, targets, num_boxes, indices=indices)
        losses.update(loss_class)
        losses.update(loss_boxes)
    ...
    def forward_enc_anchors(self, outputs, targets):
        num_boxes = sum(len(t["labels"]) for t in targets)
        num_boxes = torch.as_tensor(
            data=[num_boxes], dtype=torch.float, device=next(iter(outputs.values())).device
        )
        ...
        if self.two_stage_binary_cls:
            for bt in bin_targets:
                bt["labels"] = torch.zeros_like(bt["labels"])
        losses_enc = self.calculate_enc_anchors_loss(outputs, bin_targets, num_boxes)
        losses.update({k + f"_enc": v for k, v in losses_enc.items()})
        return losses

    
    def forward_o2m(self, outputs, targets, o2o_indices_dict=None, task_idx=0):
        """This performs the loss computation
        ...
        losses.update(self.calculate_o2m_loss(matching_outputs, targets, num_boxes, o2o_indices=o2o_indices_dict['o2o_indices'], matcher_idx=task_idx))

        # In case of auxiliary losses, we repeat this process with the output of each intermediate layer.
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                # get matching results for each image
                losses_aux = self.calculate_o2m_loss(
                    aux_outputs, targets, num_boxes, 
                    o2o_indices=o2o_indices_dict['o2o_indices'+f'_{i}'], matcher_idx=task_idx)
                losses.update({k + f"_{i}": v for k, v in losses_aux.items()})

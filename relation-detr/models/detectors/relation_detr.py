...
class RelationDETRwithLoRA(RelationDETR):
    def forward(self, images: List[Tensor], targets: List[Dict] = None):
        # get original image sizes, used for postprocess
        original_image_sizes = self.query_original_sizes(images)
        images, targets, mask = self.preprocess(images, targets)
        ...
        if denoising_groups is not None and max_gt_num_per_image is not None:
            ...
            # for lora
            dn_metas_lora = {
                "denoising_groups": denoising_groups * 2,
                "max_gt_num_per_image": max_gt_num_per_image // 2
            }
            outputs_class_lora, outputs_coord_lora = self.dn_lora_post_process(
                outputs_class_lora, outputs_coord_lora, dn_metas_lora
            )

        # prepare for loss computation
        output = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord[-1]}
        output["aux_outputs"] = self._set_aux_loss(outputs_class, outputs_coord)
        output["enc_outputs_anchors"] = {"pred_logits": enc_class, "pred_boxes": enc_coord, "anchors": enc_anchors}
        
        # prepare for lora loss computation
        num_lora = outputs_class_lora.shape[0]
        for lora_id in range(num_lora):
            output['lora'+f"_{lora_id}"] = {"pred_logits": outputs_class_lora[lora_id, -1], "pred_boxes": outputs_coord_lora[lora_id, -1]}
            output['lora'+f"_{lora_id}"]['aux_outputs'] = self._set_aux_loss(outputs_class_lora[lora_id], outputs_coord_lora[lora_id])
        ...
        if self.training:
            ...
            loss_dict = self.criterion(output, targets)
            dn_losses = self.compute_dn_loss(dn_metas, targets)
            # dn_losses_lora = self.compute_dn_loss(dn_metas_lora, targets)
            # for k, v in dn_losses_lora.items():
            #     dn_losses[k] = (dn_losses[k] + v / 2.) 
            loss_dict.update(dn_losses)
            
            # get o2o_indices (will be merged to o2m_indices)
            o2o_indices_dict = {k: v for k, v in loss_dict.items() if k.startswith('o2o_indices') and 'dn' not in k}
            loss_dict = {k: v for k, v in loss_dict.items() if not k.startswith('o2o_indices')}
            
            # compute enc loss 
            loss_enc = self.criterion.forward_enc_anchors(output['enc_outputs_anchors'], targets)
            loss_dict.update(loss_enc)
            
            # compute lora loss
            for lora_id in range(num_lora):
                loss_lora_dict = self.criterion.forward_o2m(output['lora'+f"_{lora_id}"], targets, o2o_indices_dict=o2o_indices_dict, task_idx=lora_id)
                loss_dict.update({k + f"_lora_{lora_id}": v for k, v in loss_lora_dict.items()})

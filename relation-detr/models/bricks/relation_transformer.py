...
from .lora_utils import MultiLoRALayer, ProgressiveMultiLoRALayer
class RelationTransformerDecoderLayerWithLoRA(RelationTransformerDecoderLayer):
    def __init__(
        self,
        embed_dim=256,
        d_ffn=1024,
        ...
        super().__init__(embed_dim, d_ffn, n_heads, dropout, activation, n_levels, n_points)
        self.num_lora = num_lora
        self.linear1_lora = MultiLoRALayer(embed_dim, d_ffn, lora_num=num_lora, rank=rank_lora, norm=False)
        self.linear2_lora = MultiLoRALayer(d_ffn, embed_dim, lora_num=num_lora, rank=rank_lora, norm=False)
        
        # self.linear1_lora = ProgressiveMultiLoRALayer(embed_dim, d_ffn, lora_num=num_lora, rank=rank_lora)
        # self.linear2_lora = ProgressiveMultiLoRALayer(d_ffn, embed_dim, lora_num=num_lora, rank=rank_lora)
    ...
    def forward(
        ...
        for i in range(self.num_lora):
            query_lora.append(self.forward_lora_ffn(query, i))
        query_lora = torch.stack(query_lora, dim=0)
        
        # ffn
        query = self.forward_ffn(query)

        return query, query_lora
    
class RelationTransformerDecoderWithLoRA(RelationTransformerDecoder):
    def forward(
        self,
        query,
        reference_points,
        ...
        outputs_classes, outputs_coords = [], []
        outputs_classes_lora, outputs_coords_lora = [], []
        valid_ratio_scale = torch.cat([valid_ratios, valid_ratios], -1)[:, None]
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            # # hack implement for single LoRA (TODO: support multiple LoRAs) without look_forward_twice
            # outputs_class_lora = self.class_head[layer_idx](self.norm(query_lora[0]))
            # outputs_coord_lora = self.bbox_head[layer_idx](self.norm(query_lora[0]))
            # outputs_coord_lora = outputs_coord_lora + inverse_sigmoid(reference_points).detach()
            # outputs_coord_lora = outputs_coord_lora.sigmoid()
            # outputs_classes_lora.append(outputs_class_lora)
            # outputs_coords_lora.append(outputs_coord_lora)
            
            #  Multiple LoRA support 
            outputs_classes_lora_layer, outputs_coords_lora_layer = [], []
            num_lora = self.layers[0].num_lora
            for lora_idx in range(num_lora):
                outputs_class_lora = self.class_head[layer_idx](self.norm(query_lora[lora_idx]))
                outputs_coord_lora = self.bbox_head[layer_idx](self.norm(query_lora[lora_idx]))
                outputs_coord_lora = outputs_coord_lora + inverse_sigmoid(reference_points).detach()
                outputs_coord_lora = outputs_coord_lora.sigmoid()
                outputs_classes_lora_layer.append(outputs_class_lora)
                outputs_coords_lora_layer.append(outputs_coord_lora)
            outputs_classes_lora_layer = torch.stack(outputs_classes_lora_layer, dim=0)
            outputs_coords_lora_layer = torch.stack(outputs_coords_lora_layer, dim=0)
            outputs_classes_lora.append(outputs_classes_lora_layer)
            outputs_coords_lora.append(outputs_coords_lora_layer)
        ...
        outputs_classes = torch.stack(outputs_classes)
        outputs_coords = torch.stack(outputs_coords)
        outputs_classes_lora = torch.stack(outputs_classes_lora, dim=1) # [num_lora, num_layers, batch_size, num_queries, num_classes]
        outputs_coords_lora = torch.stack(outputs_coords_lora, dim=1) # [num_lora, num_layers, batch_size, num_queries, 4]
        
        return outputs_classes, outputs_coords, outputs_classes_lora, outputs_coords_lora
    
class RelationTransformerWithLoRA(RelationTransformer):
    def forward(
        self,
        multi_level_feats,
        multi_level_masks,
        ...
        enc_outputs_class_all = enc_outputs_class
        enc_outputs_coord_all = enc_outputs_coord
        enc_anchors_all = output_proposals.sigmoid()
        ...
        outputs_classes, outputs_coords, outputs_classes_lora, outputs_coords_lora = self.decoder(
            query=target,
            value=memory,
            key_padding_mask=mask_flatten,
            reference_points=reference_points,
        ...
        if self.training:
            hybrid_classes, hybrid_coords, _, _= self.decoder(
                query=hybrid_target,
                value=memory,
                key_padding_mask=mask_flatten,
                reference_points=hybrid_reference_points,
...
class UFusionRelationTransformerEncoder(RelationTransformerEncoder):
    def __init__(self, encoder_layer: nn.Module, num_layers: int = 6):
        super().__init__(encoder_layer, num_layers)
        # self.memory_fusion = None # replace memory fusion for U-like fusion
    ...
    def forward(
        ...
        query = torch.cat(queries, -1)
        query = self.memory_fusion(query)
        return query, queries_output
        

class UFusionRelationTransformerDecoderWithLoRA(RelationTransformerDecoderWithLoRA):
    def forward(
        self,
        query,
        reference_points,
        value,
        values, # list of encoder features for U-like fusion
        spatial_shapes,
        level_start_index,
        valid_ratios,
        key_padding_mask=None,
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            query, query_lora = layer(
                ...
                query_pos=query_pos,
                reference_points=reference_points_input,
                value=values[layer_idx], # U-like fusion
                spatial_shapes=spatial_shapes,
                level_start_index=level_start_index,
                key_padding_mask=key_padding_mask,
                self_attn_mask=pos_relation,
...
class UFusionRelationTransformerWithLoRA(RelationTransformerWithLoRA):
    def __init__(
        self,
        encoder: nn.Module,
        decoder: nn.Module,
        ...
        super().__init__(encoder, decoder, num_classes, num_feature_levels, two_stage_num_proposals, hybrid_num_proposals)
        # for U-like Fusion
    ...
    def forward(
        ...
        # transformer encoder
        memory, memory_per_layer = self.encoder(
            query=feat_flatten,
            query_pos=lvl_pos_embed_flatten,
            query_key_padding_mask=mask_flatten,
            spatial_shapes=spatial_shapes,
        ...
        # copy from MI-DETR: U-like Fusion
        memory_tmp = memory_per_layer[-1]
        memory_per_layer = [self.fusion[i](torch.cat([memory_per_layer[-1], memory_per_layer[i]], dim=-1)) for i in
                            range(len(memory_per_layer) - 1)]
        ...
        outputs_classes, outputs_coords, outputs_classes_lora, outputs_coords_lora = self.decoder(
            query=target,
            value=memory,
            values=memory_per_layer, # U-like fusion
            key_padding_mask=mask_flatten,
            reference_points=reference_points,
            spatial_shapes=spatial_shapes,
            level_start_index=level_start_index,
